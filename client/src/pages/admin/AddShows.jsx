...
const AddShows = () => {
  ...
  const handleDataTimeAdd = () => {
    if (!dateTimeInput) return toast("Please select a date and time");
    const [date, time] = dateTimeInput.split("T");
    if (!date || !time) return toast("Please select a date and time");
    ...
    toast("Date and time added successfully.");
  };

  const handleDataTimeRemove = (date, time) => {
    setDateTimeSelection((prev) => {
      ...
      <div className="mt-6">
        ...
        <div className="inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg">
          ...
          <button
            ...
            className="bg-primary/80 text-white px-3 py2 text-sm rounded-lg 
                ...
                <div className="flex flex-wrap gap-2 mt-1 text-sm">
                  {time.map((time) => (
                    <div className="border border-primary px-2 py-1 flex items-center rounded">
                      <span>{time}</span>
                      <DeleteIcon
                        onClick={() => handleDataTimeRemove(date, time)}
                        width={15}
